...
def run_md_simulation(
    structure_files: dict[str, str], config: dict[str, Any]
) -> dict[str, Any]:
    """
        ...
        for temp in temperatures:
            ...
            for mdp_type in ["nvt", "npt", "md"]:
                ...
                if src_file.exists() and not dst_file.exists():
                    ...
                    logger.info(
                        f"Temperature-specific MDP file already exists: {mdp_file}"
                    )
                else:
...
def _preprocess_for_gromacs(
    pdb_filename: str, pdb_filepath: Path, config: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    gromacs_input = protonation_state(
        pdb_filename=pdb_filename, pdb_path=pdb_filepath, pH=pH
    )
...
def _setup_simulation_system(
    gromacs_files: dict[str, Any], config: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    gromacs.editconf(
        f=gromacs_files["processed_gro"], o="box.gro", c=True, d="1.0", bt="triclinic"
    )

    # Step 2: Add water
    logger.info("Adding water molecules...")
    gromacs.solvate(
        cp="box.gro", cs="spc216.gro", p=gromacs_files["topology"], o="solv.gro"
    )
    ...
    logger.info(
        f"Looking for ions.mdp in: {[os.path.join(p, 'ions.mdp') for p in gromacs.config.path]}"
    )
    ...
    gromacs.grompp(f=ions[0], c="solv.gro", p=gromacs_files["topology"], o="ions.tpr")
    ...
    gromacs.grompp(f=em, c="solv_ions.gro", p=gromacs_files["topology"], o="em.tpr")
...
def _run_multi_temp_simulations(
    system_files: dict[str, Any], config: dict[str, Any]
...
def _run_preinstalled_temp_simulation(
    temp: str,
    ...
    if simulation_time == 100:
        ...
        _modify_mdp_nsteps(
            template_path, new_mdp, int(simulation_time * 1000 * 1000 / 2)
        )
        ...
        if gpu_enabled:
            ...
            gromacs.mdrun(
                deffnm="md_" + temp + "_" + str(simulation_time), ntomp=str(n_threads)
            )
...
def _run_custom_temp_simulation(
    temp: str,
    system_files: dict[str, Any],
    simulation_time: int,
    gpu_enabled: bool,
...
def _process_trajectories(
    trajectory_files: dict[str, dict[str, str]], config: dict
) -> dict[str, dict[str, str]]:
    """
    ...
    try:
        ...
        for line in lines:
            if line.strip().startswith("ref_t"):
                ...
                modified_lines.append(
                    f"gen_temp                = {temperature}       ; temperature for Maxwell distribution\n"
                )
            else:
...
def setup_gromacs_environment(
    gromacs_path: str | None = None, mdp_dir: str | None = None
) -> None:
    """
    ...
    if mdp_dir:
        ...
        for template_file in ["ions.mdp", "em.mdp", "nvt.mdp", "npt.mdp", "md.mdp"]:
            ...
            logger.info(
                f"Template file {template_file}: {'EXISTS' if exists else 'NOT FOUND'} at {template_path}"
            )
...
def load_existing_simulation_results(
    ...
    logger.info(
        f"Successfully loaded MD simulation results for {len(trajectory_files)} temperatures"
    )
    for temp, files in trajectory_files.items():
        ...
        for temp in config.get("simulation", {}).get("temperatures", [300, 350, 400]):
            ...
            if temp_str in ["300", "310", "350", "373", "400"]:
                required_mdp_files.extend(
                    [f"nvt_{temp_str}.mdp", f"npt_{temp_str}.mdp", f"md_{temp_str}.mdp"]
                )
