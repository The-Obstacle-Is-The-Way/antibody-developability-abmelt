...
def get_required_files(
    work_dir: Path, antibody_name: str, temperatures: list[str]
) -> set[str]:
    """
...
def cleanup_temp_directory(
    ...
    if dry_run:
        ...
        if suspicious:
            logger.warning(
                f"Suspicious files (not in required or intermediate): {len(suspicious)}"
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "work_dir", type=str, help="Path to working directory (temp directory)"
    )
    parser.add_argument(
        "--antibody-name",
        type=str,
